...
SIGNED_IMAGE="nerdctl.io/seejovin93/prufwerk:latest"
...
UNSIGNED_IMAGE="nerdctl.io/seejovin93/prufwerk:${UNSIGNED_TAG}"

step "Building and pushing UNSIGNED image (new digest: ${UNSIGNED_IMAGE})..."
nerdctl build --no-cache --label "build-id=${UNSIGNED_TAG}" -t "${UNSIGNED_IMAGE}" .
nerdctl push "${UNSIGNED_IMAGE}"
